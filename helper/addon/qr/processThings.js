...
async function updateChatInMysql({
  ...
  try {
    ...
    logger.log("Error updating chat:", err);
  }
}
...
async function saveImageToFile(imageBuffer, filePath, mimetype) {
  try {
    await fs.promises.writeFile(filePath, imageBuffer);
    logger.log(`${mimetype || "IMG"} saved successfully as ${filePath}`);
  } catch (error) {
...
async function downloadMediaPromise(m, mimetype) {
  try {
    ...
    await saveImageToFile(bufferMsg, filePath, mimetype);

    return { success: true, fileName };
  } catch (err) {
    logger.log("Error in downloadMediaPromise:", err);
...
async function processBaileysMsg({ body, uid, userFromMysql, chatId }) {
  try {
    ...
    if (body.message.conversation) {
      ...
      logger.warn(`Original message ${reaction.key.id} not found for reaction`);
      return { newMessage: null, chatId };
      ...
      logger.warn("Unsupported message type in Baileys webhook");
      return null;
    }
...
async function getUserDetails(sessionId, userData) {
  try {
    ...
    logger.error("getUserDetails error:", err);
    return null;
  }
}
...
async function processMessageQr({
  ...
  try {
    ...
    if (!instanceNumber || !message.key.remoteJid || !uid) {
      logger.warn("Details not found to update chat list", {
        instanceNumber,
    ...
    logger.error("processMessageQr error:", err);
    return null;
  }
}
